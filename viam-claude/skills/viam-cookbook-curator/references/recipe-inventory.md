...
| Local Module Deployment | `references/module-development/local-deployment.md` | `viam module reload-local`, `--no-build`, `--part-id`, `viam machines part list`, `local` module type, `meta.json` `models`, `VIAM_BUILD_OS`, `VIAM_BUILD_ARCH`, CGO cross-compilation | Deploying non-registry / under-development modules to a Viam host: full `reload-local` flow, component config requirement, iteration loop, cross-arch build-on-target fallback with `--no-build` |
