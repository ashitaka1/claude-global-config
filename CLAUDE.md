...
*Keep this section updated as the project progresses. This provides operational context about where the project stands.*

Project phase: [Setup / Active Development / Maintenance / etc.]
...
### For new projects, consider creating:

- **project_spec.md** - Technical requirements, architecture, milestones, open questions, technical debt
  - Template available at: `templates/project_spec.md`
...
These agents are available in `.claude/agents/` for workflow automation:

- **pre-work-check** - Verify branch and tests before starting work
- **test-scrutinizer** - Two-phase test plan review (before and after implementation)
- **readme-updater** - Update user-facing documentation
- **claude-md-updater** - Update workflow and conventions
- **project-spec-updater** - Update technical decisions and architecture
- **changelog-updater** - Maintain changelog in Keep a Changelog format
- **completion-checker** - Pre-merge quality gate
- **retro-reviewer** - Periodic workflow optimization review
...
- **/start-feature <name>** - Create feature branch and begin guided development

Project-specific skills can be added to `.claude/skills/` in individual projects.

---

## Notes for Claude

- Delegate to agents proactively when their expertise is needed
- Use `/start-feature` to begin new features with proper workflow
- Run documentation agents in parallel when committing (readme-updater, claude-md-updater, project-spec-updater, changelog-updater)
- Consult this file for project conventions, workflows, and standards
- If project has a `project_spec.md`, use it for technical/architectural context
- Use `retro-reviewer` periodically to improve this workflow
